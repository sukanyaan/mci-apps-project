function generateTable(array) {
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      // Add the Email cell
      var emailCell = document.createElement("td");
      emailCell.innerHTML = '<button ><a href="edit.html?id='+`${object.empId}`+'">edit</a></button>';
      row.appendChild(emailCell);
  
      // Add the Email cell
      var emailCell = document.createElement("td");
      emailCell.innerHTML = '<button type="button" onclick="onDelete(' + `${object.empId}` + ')">Delete</button>';
      row.appendChild(emailCell);
